...
def to_undirected(edge_iterable, num_edges, shuffle=True):
    ...
    return np.array(neighborsMap), np.array(node_indices), np.ascontiguousarray(undirected_edges)

def get_clean_data(DATA_DIRECTORY, FILENAME, NUM_EDGES, shuffle=True):
    # if shuffle:
    #     name = os.path.join(DATA_DIRECTORY, 'wS-cleaned-shuffled.npy')
    # else:
    #     name = os.path.join(DATA_DIRECTORY, 'wS-cleaned.npy')
    # if os.path.exists(name):
    #     print('Loading from file {}'.format(name))
    #     return np.load(name)
    # else:
    #     print('Parsing from zip. Will write to file {}'.format(name))
    #     # Lets get the edges into one big array
    #     neighborsMap, edges = to_undirected(row_generator(FILENAME), NUM_EDGES, NUM_NODES, shuffle=shuffle)
    #     print('ORIGINAL DIST: {} MIN: {} MAX: {}'.format(np.abs(edges[:,0] - edges[:,1]).mean(), edges.min(), edges.max()))
    #     np.save(name, edges)
    #     return neighborsMap, edges
    # print('Parsing from zip. Will write to file {}'.format(name))
#     print('Parsing from zip.')
    # Lets get the edges into one big array
    neighborsMap, _, edges = to_undirected(row_generator(FILENAME), NUM_EDGES, shuffle=shuffle)
#     print('ORIGINAL DIST: {} MIN: {} MAX: {}'.format(np.abs(edges[:,0] - edges[:,1]).mean(), edges.min(), edges.max()))
    # np.save(name, edges)
    return neighborsMap, edges
